    ...
    public class GameController : MonoBehaviour
    {
        ...
        public GameObject GetPrefab(Object prefab)
        {
            ...
            */
            ...
            GameObject go = null;
            if (objectPool.ContainsKey(prefab.name)) {                  // If such a prefab type exists
                if(objectPool[prefab.name].Count > 0) {                     // If there is something in that list
                    go = objectPool[prefab.name][0];                            // Get reference to first element
                    objectPool[prefab.name].RemoveAt(0);                        // Remove that element from list   
                }
                else {                                                      // If no such thing in that list
                    go = Instantiate((GameObject)prefab);                       // Just create one LULZ
                }                 
            }
            else {
                Debug.Log("Creating pool for " + prefab.name + "s");
                objectPool.Add(prefab.name, new List<GameObject>());    // Create a new list of that prefab type
                go = Instantiate((GameObject)prefab);                   // Also create one LULZ
            }
        ...
        public void DestroyPrefab(GameObject aObj)
        {
            ...
            */
            ...
            if (objectPool.ContainsKey(aObj.name.Split('_')[0])) {        // Check if we are tracking such objects in the OP
                if (objectPool[aObj.name.Split('_')[0]].Count < POOL_MAX) {
                    objectPool[aObj.name.Split('_')[0]].Add(aObj);
                    Debug.Log("Adding " + aObj.name + " to pool of " + aObj.name.Split('_')[0]);
                }
                else {
                    Destroy(aObj);
                }
